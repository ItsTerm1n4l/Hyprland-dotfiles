...
SCRIPTDIR=$(dirname "$0")
ICONSDIR="/home/pi/.icons/crayon" #"$DIRNAME"
DIR="$HOME/.config"
...
FOLDERS () {
FOLDERSYELLOW="#ffcc57"
FOLDERSORANGE="#f18831"
convert "$ICONSDIR"/scalable/icons_ref/folder.png -fuzz 15% -fill "${YELLOW}" -opaque "${FOLDERSYELLOW}" "$ICONSDIR"/.filesWIP.png
convert "$ICONSDIR"/.filesWIP.png -fuzz 15% -fill "${ALTYELLOW}" -opaque "${FOLDERSORANGE}" "$ICONSDIR"/scalable/places/folder.png
rm "$ICONSDIR"/.filesWIP.png
}
